...
#define _INTERVAL_DIST   21 // USS interval (unit: msec)
#define _INTERVAL_SERVO   20 // servo interval (unit: msec)
#define _INTERVAL_SERIAL  22 // serial interval (unit: msec)
...
void loop() {
  ...
  if(time_curr >= (last_sampling_time_servo + _INTERVAL_SERVO)) {
        ...
        event_servo = true;
        last_sampling_time_servo-=1;
  }
  if(time_curr >= (last_sampling_time_serial + _INTERVAL_SERIAL)) {
        last_sampling_time_serial += _INTERVAL_SERIAL;
        event_serial = true;
        last_sampling_time_serial -= 2;
  }

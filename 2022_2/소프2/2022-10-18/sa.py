...
class Calculator(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        # Digit Buttons
        self.digitButton = [x for x in range(0, 10)]  #class 이용해서 줄 수 줄임
        for i in range(10):
            x = str(i+1)
            self.digitButton[i] = Button(x ,self.buttonClicked)
        ...
        for i in range(2,-1,-1):
            for j in range(0,3):
                numLayout.addWidget(self.digitButton[(2-i)*3+j+1],i,j)
